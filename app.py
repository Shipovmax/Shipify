...
class SpotifyManagerApp(ctk.CTk):
    """Main application window for the Spotify Library Manager GUI."""
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(settings, text="Time range:", font=ctk.CTkFont(size=13)).grid(
            row=0, column=0, padx=(15, 8), pady=15, sticky="w"
        )
        self._time_var = ctk.StringVar(value="All time")
        ctk.CTkOptionMenu(
            settings,
        ...
        ctk.CTkLabel(settings, text="Track count:", font=ctk.CTkFont(size=13)).grid(
        ...
        ctk.CTkLabel(playlist_frame, text="Playlist:").grid(
        ...
        ctk.CTkButton(
            playlist_actions,
            text="Add Top Tracks to Playlist",
            fg_color=self.SPOTIFY_GREEN,
            hover_color=self.SPOTIFY_GREEN_HOVER,
        ...
        ctk.CTkLabel(
            log_header, text="Operation Log", font=ctk.CTkFont(size=13, weight="bold")
        ).pack(side="left")
        ctk.CTkButton(
    ...
    def _run_async(
        ...
        *args: Any,
        ...
        """Run an operation on a background thread while managing busy-state.

        Args:
            target: Callable to execute off the main (UI) thread.
            *args: Positional arguments forwarded to `target`.
            on_done: Optional callback invoked on the main thread with the
                result of `target`, once it completes successfully.
        """
        if self._busy:
            self._log("Wait for the current operation to finish", "warning")
            return
        ...
        def worker() -> None:
            try:
                result = target(*args)
                if on_done:
                    # default arg captures the value at lambda creation time
                    self.after(0, lambda r=result: on_done(r))
            except Exception as exc:
                logging.exception("Operation failed")
                err_msg = str(exc)
                self.after(0, lambda msg=err_msg: self._log(f"Error: {msg}", "error"))
            finally:
                self.after(
    ...
    def _init_spotify_async(self) -> None:
        self._log("Initializing Spotify...")

        def on_done(result: tuple[SpotifyClient, dict[str, Any]]) -> None:
            client, user = result
            self._client = client
            name = user.get("display_name") or user.get("id", "?")
            self._user_label.configure(text=f"Connected: {name}")
            self._log(f"Connected as {name}")
            ...
            try:
                ...
                self.after(
                    ...
                    lambda msg=err_msg: self._log(
                        f"Authorization error: {msg}", "error"
                    ),
                )
            finally:
    ...
    def _check_ready(self) -> bool:
        if not self._client:
            messagebox.showwarning("Please wait", "Spotify is still connecting")
            return False
        return True
    ...
    def _get_inputs(self) -> tuple[str, int] | None:
        try:
            ...
            if not 1 <= limit <= 200:
                raise ValueError("Track count must be between 1 and 200")
        except ValueError as exc:
            messagebox.showerror("Error", f"Invalid track count: {exc}")
            return None
    ...
    def _preview_top(self) -> None:
        ...
        self._log(f"Loading top {limit} ({self._time_var.get()})...")
        client = self._client
        assert client is not None  # guaranteed by _check_ready

        def fetch() -> list[Track]:
            return client.get_top_tracks(time_range, limit)

        def on_done(tracks: list[Track]) -> None:
            self._cached_top_tracks = tracks
            self._log(f"Fetched {len(tracks)} tracks")
            if not tracks:
                messagebox.showinfo("Empty", "No top tracks returned")
                return
            PreviewWindow(
                self,
                tracks,
                on_confirm=self._add_selected_to_library,
                title=f"Top Tracks ({self._time_var.get()})",
                confirm_text="Add to Library",
            )
    ...
    def _quick_add_to_library(self) -> None:
        ...
        self._log(f"Quick-adding top {limit}...")
        client = self._client
        assert client is not None

        def task() -> OperationResult:
    ...
    def _add_selected_to_library(self, track_ids: list[str]) -> None:
        ...
        self._log(f"Adding {len(track_ids)} selected tracks to the library...")
    ...
    def _on_add_done(self, result: OperationResult) -> None:
        self._log(
            ...
            f"already present: {result.skipped}, errors: {result.errors}"
        )
    ...
    def _refresh_playlists_async(self) -> None:
        ...
        def on_done(playlists: list[dict[str, Any]]) -> None:
            ...
            self._log(f"Loaded playlists: {len(playlists)}")
    ...
    def _create_playlist_dialog(self) -> None:
        ...
        def task() -> dict[str, str]:
            return client.create_playlist(
                clean_name,
                public=False,
                description="Created by Spotify Library Manager",
            )

        def on_done(playlist: dict[str, str]) -> None:
            self._log(f"Created playlist: {playlist['name']}")
            self._refresh_playlists_async()
    ...
    def _add_top_to_playlist(self) -> None:
        ...
        if not playlist_id:
            messagebox.showwarning("Playlist", "Select a playlist")
            return
        inputs = self._get_inputs()
        if not inputs:
            return
        ...
        self._log(f"Loading top tracks for playlist '{playlist_label}'...")

        def fetch() -> list[Track]:
            return client.get_top_tracks(time_range, limit)

        def on_fetched(tracks: list[Track]) -> None:
            if not tracks:
                self._log("Top tracks list is empty")
                return

            def on_confirm(selected_ids: list[str]) -> None:
                def add_task() -> OperationResult:
                ...
                def on_added(result: OperationResult) -> None:
                    self._log(
                        f"Added to playlist: {result.added}, "
                        f"duplicates skipped: {result.skipped}, errors: {result.errors}"
                    )
                    self._refresh_playlists_async()
            ...
            PreviewWindow(
                ...
                title=f"Adding to '{playlist_label}'",
                confirm_text="Add to Playlist",
            )
    ...
    def _clear_library_dialog(self) -> None:
        ...
        dlg = ctk.CTkInputDialog(
            text='Type "DELETE" to confirm full removal:',
        ...
        def on_done(result: OperationResult) -> None:
            self._log(
                f"Library cleared. Removed: {result.removed}, errors: {result.errors}"
            )
